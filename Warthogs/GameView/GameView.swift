...
struct GameView: View {
    ...
    var body: some View {
        ...
        .onChange(of: sequenceManager.playerHealth.currentHealth) {_, newHealth in
            if newHealth <= 0 {
                showLoseScreen = true
            }
        }
        .onChange(of: sequenceManager.enemyHealth.currentHealth) { _, newHealth in
            if newHealth <= 0 {
                showWinScreen = true
            }
        }
        .fullScreenCover(isPresented: $showWinScreen){
            WinningUIView(health: HealthManager())
        }
    
        .fullScreenCover(isPresented: $showWinScreen){
            LosingUIView(health: HealthManager())
        }
